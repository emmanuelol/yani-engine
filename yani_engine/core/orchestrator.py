            ...
            if task_type == "validation":
                ...
                try:
                    ...
                    for garbage_file in post_untracked - pre_untracked:
                        ...
                        if abs_path.startswith(cwd_real) and os.path.exists(abs_path) and not garbage_file.startswith(".yani/"):
    ...
    async def batch_diff_review(self, wave_tmp_files: list):
        ...
        for tmp_path in wave_tmp_files:
            ...
            if actual_filename in rejected_files or basename in rejected_files:
                ...
                encoded_path = actual_filename.replace("/", "__").replace(":", "__colon__")
                possible_rollback = os.path.join(".yani", "rollbacks", task_id, encoded_path) if task_id else None
    ...
    async def run(self, command: str, args: list):
        ...
        if command == "resume":
            ...
            if choice == "B":
                ...
                await flush_task_registry()
                console.print("[yellow]Tasks demoted to pending. Please run /yani-engine:rollback to revert file changes manually.[/yellow]")
                return
            elif choice == "S":
                for t_id in interrupted:
        ...
        try:
            ...
            if command == "execute":
                            ...
                            async def worker(worker_id: str):
                                while True:
                                    ...
                                    try:
                                        ...
                                        from yani_engine.core.state import flush_task_registry
                                        await flush_task_registry()
                ...
                if not completed_changes:
                    console.print("[yellow]No completed changes found. Run /yani-engine:status to see pending tasks.[/yellow]")
                    return
...
"""
                ...
                # 3. Print Header
                print(f"\nyani-engine — Session {last_session_id} | Budget: {pct_used}% used ({tokens}/{limit} est. tokens)")
                ...
                if not os.path.exists(know_path):
                    know_str = f"Knowledge: no registry — /yani-engine:start creates it"
                else:
                ...
                console.print("\n[bold green]Tasks successfully queued! Run /yani-engine:execute to trigger the LLM patch wave.[/bold green]")
                return
                ...
                if len(prompt_text) < 20:
                    print("Error: /yani-engine iterate requires a detailed prompt (min 20 chars). Vague prompts cause hallucinated task loops.")
                    return
    ...
    async def _archive_stale_sessions(self):
        ...
        os.makedirs(".yani/archive", exist_ok=True)
        ...
        for sid, sess_line, _ in to_archive:
            ...
            archive_row = f"| {sid} | {datetime.utcnow().isoformat()}Z | .yani/archive/{sid}.md | {len(archived_tasks_per_session[sid])} | {sess_line.split('|')[5].strip()} |"
            if idx_start == -1:
                new_lines.append("")
        ...
        print(f"Archived {len(to_archive)} session(s) → .yani/archive/ ({len(lines) - len(final_lines)} lines trimmed from memory.md)")
